...
--changeset OrestHutovych:add-twitter-user_profiles-table-column-edited_timestamp
--comment add column edited_timestamp to twitter.tweets table
alter table twitter.tweets
    add column edited_timestamp timestamp;

update twitter.tweets
set edited_timestamp = created_timestamp
where edited_timestamp is null;

alter table twitter.tweets
    alter column edited_timestamp set not null;
--rollback alter table twitter.tweet drop column edited_timestamp;